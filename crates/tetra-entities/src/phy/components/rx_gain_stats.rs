...
impl RxGainExportWriter {
    ...
    pub fn append_window(&self, row: &RxGainWindowExport) -> Result<(), String> {
        ...
        let line = format!(
            ...
            row.crc_ok,
            row.false_positive,
            row.crc_pass_rate * 100.0,
            row.false_positive_rate * 100.0,
            row.slot0_detected,
            row.slot0_crc_pass_rate * 100.0,
            row.gaps_detected,
            row.gap_rate * 100.0,
    ...
    pub fn write_summary(&self, rows: &[RxGainSummaryExport]) -> Result<(), String> {
        ...
        for row in rows {
            let line = format!(
                ...
                row.slot0_crc_pass_rate * 100.0,
                row.slot1_crc_pass_rate * 100.0,
                row.slot2_crc_pass_rate * 100.0,
                row.slot3_crc_pass_rate * 100.0,
                row.gaps_detected,
                row.gap_rate * 100.0,
                row.stability_status,
                row.duration_ms,
